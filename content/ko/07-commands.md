...
# 또는 짧게 (별칭)
claude --model opus      # 최신 Opus (현재 Opus 5)
claude --model sonnet    # 최신 Sonnet (현재 Sonnet 5)
claude --model haiku     # 가장 빠르고 저렴 (Haiku 4.5)
claude --model fable     # 가장 어려운 장시간 작업용 (Fable 5)
claude --model opusplan  # 계획은 Opus, 실행은 Sonnet
```

> **2026년 8월 기준 모델 라인업**: Fable 5 → Opus 5 → Sonnet 5 → Haiku 4.5.
> 별칭(`opus`, `sonnet`)은 시간이 지나면 최신 버전을 가리키도록 바뀌므로, 버전을 고정하려면 `claude-opus-5`처럼 정식 모델명을 쓰세요.
...
| `/usage` | 현재 세션의 토큰 사용량·비용 확인 (`/cost`는 같은 명령의 별칭) |
| `/effort` | 모델의 사고 강도 조절 (low/medium/high/xhigh/max) |
...
| `/vim` | Vim 편집 모드 켜기/끄기 |
| `/theme` | 색상 테마 변경 |
| `/branch` | 현재 지점에서 대화를 분기 (다른 방향을 시도해도 원본 유지) |
| `/fork` | 현재 대화를 백그라운드 세션으로 복사하고 여기서 계속 작업 |
| `/rewind` | 코드와 대화를 이전 체크포인트로 되돌리기 |
| `/context` | 컨텍스트 창 토큰 사용량 시각화 |
| `/rename [이름]` | 세션에 이름 붙이기 (`--resume 이름`으로 재개) |
| `/goal` | 목표를 정해두면 조건이 충족될 때까지 Claude가 계속 작업 |
| `/subtask` | 곁가지 작업을 서브에이전트에 맡기고 결과만 받기 |
...
현재 지점에서 대화를 분기합니다. 복잡한 리팩토링이나 구조적인 변경을 시도하기 전에 분기해두면, 실패해도 지금까지의 대화를 잃지 않습니다.
...
### `/fork` — 복사본을 백그라운드로 넘기기

현재 대화를 백그라운드 세션으로 복사하고, 지금 터미널에서는 하던 작업을 계속합니다. 한 갈래는 백그라운드에서 굴리면서 다른 갈래를 직접 진행할 때 유용합니다.

```
> /fork
→ 대화 복사본이 백그라운드 세션으로 이동
→ 현재 창에서는 그대로 작업 계속
```

> 실수한 지점으로 아예 되돌리고 싶다면 `/rewind`(또는 `Esc` 두 번)를 쓰세요. 코드와 대화를 함께 이전 체크포인트로 복원합니다.

### `/context` — 컨텍스트 사용량 확인

```
> /context
→ [■■■■■■■□□□] 70% (700,000 / 1,000,000 tokens)
```

현재 대화가 컨텍스트 창의 몇 %를 차지하는지 시각적으로 보여줍니다. 80% 이상 차면 `/compact`나 `/clear`를 고려하세요.

> 컨텍스트 창은 모델에 따라 다릅니다. Fable 5 / Opus 5 / Sonnet 5는 **100만(1M) 토큰**, Haiku 4.5는 20만 토큰입니다.
...
현재 세션에서 얼마나 썼는지 확인합니다. (`/cost`는 같은 명령의 별칭입니다. 구독 사용자는 여기서 플랜 사용량 막대도 함께 볼 수 있고, `/stats`로 세션 통계를 볼 수도 있습니다.)
...
대화 중 입력:
  ...
  /compact            → 대화 압축 (자주 쓰기!)
  /clear              → 대화 초기화
  /branch             → 현재 지점에서 대화 분기
  /fork               → 대화 복사본을 백그라운드 세션으로
  /rewind             → 코드·대화를 체크포인트로 되돌리기
  /context            → 컨텍스트 사용량 확인
  /usage (= /cost)    → 사용량·비용 확인
  /model              → 모델 변경
  /effort             → 사고 강도 조절
  /rename [이름]       → 세션 이름 붙이기
  /bug                → 버그 리포트 전송
