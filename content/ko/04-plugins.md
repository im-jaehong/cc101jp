...
| **바선생** (vibe-sunsang) | Claude Code 대화 로그를 자동 수집해 요청 품질을 A~D로 평가하고 월간 성장 보고서 생성. AI 사용 패턴을 분석해 더 잘 쓰는 법을 코칭 | `/vibe-sunsang:help` |
| **deep-research-kit** | 멀티에이전트 7단계 리서치 자동화. 웹/학술/기술 출처 병렬 수집 → 교차검증 → 보고서 생성. 할루시네이션 없이 심층 조사 | `/deep-research [주제]` |
| **pumasi (품앗이)** | Claude(PM) + Codex(병렬 개발자) 조합. 독립 모듈을 여러 Codex 인스턴스가 동시 구현 → Claude가 통합 검증. 대규모 작업 속도 향상 | `/pumasi [작업 설명]` |
